...
"""
from apps.themes.models import Layout, StyleClass, Template, Theme, Typography
...
from .serializers import (
    LayoutPublicSerializer,
    StyleClassPublicSerializer,
    TemplatePublicSerializer,
    ThemePublicSerializer,
    TypographyPublicSerializer,
)
...
class ThemePublicViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def render_template(self, request, pk=None):
        ...
        if not template_slug:
            return Response(
                {"error": "template_slug is required"}, status=status.HTTP_400_BAD_REQUEST
            )

        try:
            # Get template from theme
            template = Template.objects.filter(
                theme=theme, slug=template_slug, is_active=True
            ).first()
    ...
    def render_page(self, request, pk=None):
        ...
        if not page_data:
            return Response({"error": "page_data is required"}, status=status.HTTP_400_BAD_REQUEST)
...
class TemplatePublicViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return Template.objects.filter(theme__is_active=True, is_active=True).select_related(
            "theme"
        )
...
class LayoutPublicViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return Layout.objects.filter(theme__is_active=True, is_active=True).select_related("theme")
...
class StyleClassPublicViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return StyleClass.objects.filter(theme__is_active=True, is_active=True).select_related(
            "theme"
        )
...
class TypographyPublicViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return Typography.objects.filter(theme__is_active=True, is_active=True).select_related(
            "theme"
        )
