...
class TranslationIntegrationTest(TestCase):
    def setUp(self):
        ...
        self.store = Store.objects.create(
            name="Test Store",
            slug="test-store",
            owner=User.objects.create_user(email="test@example.com", password="testpass123"),
        )

        # Create test languages
        self.en_lang = Language.objects.create(code="en", name="English", is_default=True)

        self.es_lang = Language.objects.create(code="es", name="Spanish", is_default=False)
    ...
    def test_translation_service_fallback_text(self):
        ...
        result = TranslationService.get_translation(
            "nonexistent.key", "en", self.store, "Fallback Text"
        )
    ...
    def test_bulk_create_translations(self):
        ...
        self.assertTrue(
            Translation.objects.filter(
                store=self.store, key__key__in=["bulk.key1", "bulk.key2"]
            ).exists()
        )
    ...
    def test_translate_text_method(self):
        ...
        TranslationService.create_translation(
            key=auto_key, language_code="es", text="Texto de prueba", store=self.store
        )
