...
from apps.public.translations.models import Translation, TranslationKey, Language
...
def test_translation_store_isolation(store_a, store_b, language):
    ...
    trans_a = Translation.objects.create(store=store_a, key=key, language=language, value='Welcome A')
    trans_b = Translation.objects.create(store=store_b, key=key, language=language, value='Welcome B')
