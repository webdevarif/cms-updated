...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if "error" in stats:
            self.stdout.write(self.style.ERROR(f"Error getting cache stats: {stats['error']}"))
            return
        ...
        if stats["hit_rate"] < 50:
            self.stdout.write(self.style.WARNING(f"Low cache hit rate: {stats['hit_rate']:.2f}%"))
        else:
            self.stdout.write(self.style.SUCCESS(f"Good cache hit rate: {stats['hit_rate']:.2f}%"))
