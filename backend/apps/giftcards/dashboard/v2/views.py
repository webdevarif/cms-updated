...
class DashboardGiftCardViewSet(viewsets.ModelViewSet):
        ...
        try:
            with transaction.atomic():
                if action_type == 'activate':
                    ...
                    for giftcard in queryset.filter(status='active'):
                        try:
                            GiftCardService.extend_gift_card_expiry_days(giftcard, days, request.user)
                            extended_count += 1
                        except Exception:
                ...
                log_event_async.delay({
                    ...
                    'metadata': {
                        ...
                        'affected_count': updated if 'updated' in locals() else deleted if 'deleted' in locals() else extended_count if 'extended_count' in locals() else voided_count if 'voided_count' in locals() else 0,
                ...
                return Response({
                    ...
                    'found': total_found,
                    'affected': updated if 'updated' in locals() else deleted if 'deleted' in locals() else extended_count if 'extended_count' in locals() else voided_count if 'voided_count' in locals() else 0
                })
    ...
    def activate(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                DashboardGiftCardSerializer(gift_card).data,
    ...
    def analytics(self, request):
        ...
        try:
            ...
            total_value_created = giftcards.aggregate(
                total=Sum('initial_balance')
            )['total'] or 0
            
            remaining_value = giftcards.filter(status='active').aggregate(
                total=Sum('current_balance')
            )['total'] or 0
            
            redeemed_value = giftcards.filter(status='used').aggregate(
                total=Sum('initial_balance')
            )['total'] or 0
            
            avg_giftcard_value = giftcards.aggregate(
                avg=Avg('initial_balance')
            )['avg'] or 0
            ...
            if start_date and end_date:
                ...
                daily_redemptions = giftcards.filter(status='used').annotate(
                    date=TruncDate('updated_at')
                ).values('date').annotate(
                    count=Count('id'),
                    value=Sum('initial_balance')
                ).order_by('date')
            ...
            # Top recipients
            top_recipients = giftcards.values('recipient_email').annotate(
                count=Count('id'),
                total_value=Sum('initial_balance')
            ).exclude(recipient_email='').order_by('-total_value')[:10]
            ...
            # Sender analysis
            top_senders = giftcards.values('sender_email').annotate(
                count=Count('id'),
                total_value=Sum('initial_balance')
            ).exclude(sender_email='').order_by('-total_value')[:10]
            ...
            analytics_data = {
                'overview': {
                    ...
                    'redemption_rate': (used_giftcards / total_giftcards * 100) if total_giftcards > 0 else 0
                },
                'financial': {
