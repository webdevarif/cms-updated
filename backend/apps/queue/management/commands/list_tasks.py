...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--status', type=str, choices=['active', 'scheduled', 'failed'], help='Filter by status')
    ...
    def handle(self, *args, **options):
        ...
        if status == 'active':
            ...
            for task in tasks:
                self.stdout.write(f"  - {task['task_id']}: {task['name']} (worker: {task['worker']})")
