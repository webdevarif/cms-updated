        ...
        try:
            with transaction.atomic():
                if action_type == 'publish':
                    ...
                    message = f'Successfully removed tags {tags_to_remove} from {success_count} posts'
                    ...
                    for post in queryset:
                        try:
                            ...
                            results.append({'id': post.id, 'status': 'success', 'duplicate_id': duplicate_post.id})
                        except Exception as e:
                            results.append({'id': post.id, 'status': 'error', 'error': str(e)})
                    ...
                    for post in queryset:
                        try:
                            ...
                            results.append({'id': post.id, 'status': 'success', 'new_status': new_status})
                        except Exception as e:
                            results.append({'id': post.id, 'status': 'error', 'error': str(e)})

                    success_count = sum(1 for r in results if r['status'] == 'success')
                    message = f'Successfully changed status to {new_status} for {success_count} posts'
        ...
        responses={
            200: OpenApiResponse(
                ...
                examples=[{
                    ...
                    'authors': [
                        {'email': 'author@example.com', 'posts_count': 45}
                    ],
                    'publishing_trends': [
    ...
    def analytics(self, request):
        ...
        try:
            ...
            if hasattr(Post, 'post_type'):
                ...
                posts_by_type = {item['post_type__name']: item['count'] for item in type_counts if item['post_type__name']}
            ...
            if hasattr(Post, 'tags'):
                from django.db.models import Count
                tag_counts = base_queryset.exclude(tags__isnull=True).exclude(tags__exact='').values_list('tags', flat=True)
                tag_dict = {}
        ...
        try:
            ...
            if start_dt or end_dt:
                ...
                analytics = {
                    ...
                    'comments_by_day': queryset.extra(
            ...
            if approved_only:
                analytics = {k: v for k, v in analytics.items() if 'approved' in k.lower() or 'total' in k.lower()}
