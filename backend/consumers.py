...
class NotificationsConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        ...
        logger.info(
            f"User {self.user.id if self.user else 'unknown'} disconnected from notifications"
        )
    ...
    def get_unread_notifications(self):
        ...
        try:
            notifications = Notification.objects.filter(user=self.user, is_read=False).order_by(
                "-created_at"
            )[
                :10
...
class SearchConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def connect(self):
        ...
        await self.channel_layer.group_add(self.search_group, self.channel_name)

        await self.accept()
        logger.info(
            f"User {self.user.id if self.user else 'anonymous'} connected to search WebSocket"
        )
    ...
    async def disconnect(self, close_code):
        ...
        if hasattr(self, "search_group"):
            await self.channel_layer.group_discard(self.search_group, self.channel_name)
    ...
    def get_live_suggestions(self, query, limit):
        ...
        try:
            suggestions = SearchService.get_suggestions(store=self.store, query=query, limit=limit)
