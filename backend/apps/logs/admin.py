...
class LogEntryAdmin(admin.ModelAdmin):
    ...
    list_display = ['store', 'event_type', 'level', 'user', 'ip_address', 'is_suspicious', 'created_at']
    ...
    fieldsets = (
        ('Event Information', {
            'fields': ('event_type', 'level', 'message')
        }),
        ('User & Request', {
            'fields': ('user', 'session_id', 'ip_address', 'user_agent', 'request_id')
        }),
        ('Event Details', {
            'fields': ('entity_type', 'entity_id', 'metadata')
        }),
        ('Analytics', {
            'fields': ('page_url', 'referrer', 'duration_ms')
        }),
        ('Security', {
