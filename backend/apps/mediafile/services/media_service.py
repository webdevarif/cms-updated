...
class MediaService:
    ...
    def upload_file(self, store, file_obj, uploaded_by=None, folder=None, resource_type=None, alt_text="", description="", metadata=None):
        """
                    ...
                    if hasattr(file_obj, 'temporary_file_path'):
                        ...
                        self.s3_client.upload_file(
                            ...
                            ExtraArgs={
                                ...
                                'ACL': 'public-read' if determined_type in ['image', 'video'] else 'private'
                            }
                        )
                    else:
                        # File is in memory
    ...
    def validate_file_upload(file_obj, store, folder=None):
        ...
        current_usage = MediaCRUDService.filter(store=store).aggregate(
            total=models.Sum('file_size')
        )['total'] or 0
        
        max_storage = getattr(settings, 'MAX_STORE_STORAGE', 5 * 1024 * 1024 * 1024)  # 5GB default
        if current_usage + file_size > max_storage:
            errors['quota'] = f'Storage quota exceeded. Available: {max_storage - current_usage} bytes'
    ...
    def search_media_files(store, query, resource_type=None, folder=None):
        ...
        return MediaCRUDService.filter(**filters).filter(
    ...
    def get_storage_usage(store):
        ...
        # Total usage
        total_usage = files.aggregate(
            total_size=Sum('file_size'),
            total_files=Count('id')
        )
        
        # Usage by type
        usage_by_type = files.values('resource_type').annotate(
            size=Sum('file_size'),
            count=Count('id')
        ).order_by('-size')
        
        # Usage by folder
        usage_by_folder = files.values('folder__name').annotate(
            size=Sum('file_size'),
            count=Count('id')
        ).order_by('-size')
    ...
    def optimize_images(store, quality=80, max_width=1920, max_height=1080):
        ...
        for image in images:
            try:
                ...
                logger.info(f"Would optimize image {image.original_filename} "
    ...
    def generate_presigned_upload_url(store, filename, resource_type, folder=None, expires_in=3600):
        ...
        safe_filename = f"{slugify(base_filename)}_{timezone.now().strftime('%Y%m%d_%H%M%S')}{file_extension}"
        ...
        response = media_service.s3_client.generate_presigned_post(
            ...
            Conditions=[
                ...
                ["content-length-range", 1, getattr(MediaFile, f"{resource_type.upper()}_MAX_SIZE")]
            ]
        )
