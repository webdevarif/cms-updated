...
class TranslationDashboardViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Translation.objects.filter(store__owner=self.request.user).select_related(
            "language", "translation_key", "store"
        )
    ...
    @extend_schema(summary="List translations", description="Get all translations for owned stores")
    def list(self, request, *args, **kwargs):
        """List translations"""
        return super().list(request, *args, **kwargs)

    @extend_schema(summary="Create translation", description="Create new translation for store")
    def create(self, request, *args, **kwargs):
        """Create translation"""
        return super().create(request, *args, **kwargs)

    @extend_schema(summary="Update translation", description="Update existing translation")
    def update(self, request, *args, **kwargs):
        """Update translation"""
        return super().update(request, *args, **kwargs)

    @extend_schema(summary="Delete translation", description="Delete translation")
    ...
    def bulk_import(self, request):
        ...
        if not translations_data:
            return Response(
                {"error": "No translation data provided"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        for translation_data in translations_data:
            try:
                ...
                if serializer.is_valid():
                    ...
                    errors.append({"data": translation_data, "errors": serializer.errors})
            except Exception as e:
                errors.append({"data": translation_data, "error": str(e)})
        ...
        for update_data in updates:
            ...
            try:
                ...
                if serializer.is_valid():
                    ...
                    errors.append({"id": translation_id, "errors": serializer.errors})
            except Translation.DoesNotExist:
                errors.append({"id": translation_id, "error": "Translation not found"})
            except Exception as e:
                errors.append({"id": translation_id, "error": str(e)})

        return Response(
            {"updated": updated_count, "errors": errors, "total_processed": len(updates)}
        )
    ...
    def bulk_delete(self, request):
        ...
        if not translation_ids:
            return Response(
                {"error": "No translation IDs provided"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response({"deleted": deleted_count, "requested": len(translation_ids)})
    ...
    def analytics(self, request):
        ...
        try:
            ...
            base_queryset = Translation.objects.filter(store__owner=user, **date_filter)
            ...
            languages = (
                base_queryset.values("language__code", "language__name")
                ...
                language_data.append(
                    {
                        ...
                        "completion_rate": (lang["active_count"] / lang["count"] * 100)
                        if lang["count"] > 0
                        else 0,
                    }
                )
            ...
            translation_keys = (
                base_queryset.values("translation_key__key")
                .annotate(
            ...
            stores = (
                base_queryset.values("store__name", "store__id")
                .annotate(
                    total_translations=Count("id"),
            ...
            analytics_data = {
                "overview": {
                    ...
                    "completion_rate": (active_translations / total_translations * 100)
                ...
                "stores": store_data,
                "trends": {"activity": translation_trends},
                "quality_metrics": {
                    "avg_translations_per_key": total_translations / len(key_data)
                    if key_data
                    else 0,
