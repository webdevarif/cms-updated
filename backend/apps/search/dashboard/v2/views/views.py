...
from .serializers import (
    SearchResultSerializer, SearchSuggestionSerializer,
    SearchAnalyticsSerializer, SearchIndexSerializer, SearchQuerySerializer
)
...
class SearchViewSet(viewsets.ViewSet):
    ...
    def analytics(self, request):
        ...
        try:
            ...
            queries = SearchQuery.objects.filter(store=store, **date_filter).order_by('-created_at')[:1000]  # Increased limit for better analytics
            ...
            zero_results_rate = (queries.filter(results_count=0).count() / total_queries * 100) if total_queries > 0 else 0

            # Search trends over time
            search_trends = []
            if start_date and end_date:
                from django.db.models.functions import TruncDate
                daily_searches = queries.annotate(
                ...
                search_trends = [{
                    ...
                    'success_rate': ((item['total_queries'] - item['zero_results']) / item['total_queries'] * 100) if item['total_queries'] > 0 else 0
                } for item in daily_searches]

            # Content type popularity (based on search results)
            content_type_popularity = queries.values('search_type').annotate(
                count=Count('id'),
                avg_results=Avg('results_count')
            ).exclude(search_type='').order_by('-count')
            ...
            user_engagement = {
                'total_search_sessions': queries.values('user').distinct().count() if queries.filter(user__isnull=False).exists() else 0,
                'avg_searches_per_user': total_queries / max(queries.values('user').distinct().count(), 1),
