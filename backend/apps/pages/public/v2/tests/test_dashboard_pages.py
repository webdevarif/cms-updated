...
class DashboardPagesAPITests(APITestCase):
    ...
    def test_bulk_actions(self):
        ...
        data = {"action": "publish", "page_ids": [self.page.id, page2.id]}
    ...
    def test_page_with_categories_and_tags(self):
        ...
        tag_taxonomy = Taxonomy.objects.create(
            name="Tags", slug="tags", taxonomy_type="tag", store=self.store
        )
        ...
        tag_term = Term.objects.create(
            name="test-tag", slug="test-tag", taxonomy=tag_taxonomy, store=self.store
        )
