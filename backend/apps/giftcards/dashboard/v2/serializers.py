...
class DashboardGiftCardSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'gift_card_type', 'status', 'recipient_name', 'recipient_email',
            'sender_name', 'sender_email', 'message', 'expires_at',
            'is_expired', 'is_redeemable', 'metadata', 'store',
            'created_by', 'created_by_email', 'created_by_name',
            'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'code', 'initial_balance', 'current_balance', 'currency',
            'created_at', 'updated_at', 'created_by', 'created_by_email', 'created_by_name'
        ]
...
class DashboardGiftCardCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = GiftCard
        fields = [
            'initial_balance', 'currency', 'gift_card_type', 'recipient_name',
            'recipient_email', 'sender_name', 'sender_email', 'message',
            'expires_at', 'metadata'
        ]
...
class DashboardGiftCardBulkCreateSerializer(serializers.Serializer):
    ...
    gift_cards = serializers.ListField(
...
class DashboardGiftCardHistorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'action', 'amount', 'order', 'notes', 'metadata',
            'created_by', 'created_by_email', 'created_by_name', 'created_at'
        ]
        read_only_fields = [
            'id', 'created_at', 'created_by', 'created_by_email', 'created_by_name'
        ]
    ...
    def validate(self, data):
        ...
        if not data.get('days') and not data.get('new_expiry_date'):
            raise serializers.ValidationError(
                "Either 'days' or 'new_expiry_date' must be provided"
            )
...
class BulkActionSerializer(serializers.Serializer):
    ...
    action = serializers.ChoiceField(choices=[
        'activate', 'void', 'delete', 'extend_expiry', 'archive', 'restore', 'status_change'
    ])
