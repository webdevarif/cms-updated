...
class QueueService:
    ...
    def enqueue_task(task_name, args=None, kwargs=None, countdown=0, eta=None):
        ...
        task = current_app.send_task(
            task_name, args=args or [], kwargs=kwargs or {}, countdown=countdown, eta=eta
        )
        ...
        if result.failed():
            ...
            if task_name:
                QueueService.enqueue_task(
                    task_name, args=result.args, kwargs=result.kwargs, countdown=countdown
                )
...
class TaskMonitoringService:
    ...
    def get_task_stats(hours=24):
        ...
        stats = {
            ...
            "avg_duration": logs.filter(status="success").aggregate(avg=models.Avg("duration_ms"))[
                "avg__duration"
            ]
            or 0,
        }
    ...
    def get_slow_tasks(threshold_ms=5000):
        ...
        return TaskLog.objects.filter(duration_ms__gt=threshold_ms).order_by("-duration_ms")
    ...
    def get_failing_tasks(limit=50):
        ...
        return (
            TaskLog.objects.filter(status="failed")
            .values("task_name")
