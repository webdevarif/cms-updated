...
export default function PostsPage() {
  ...
  const { data: posts, error, isLoading } = usePosts(storeId);
  const { data: postTypes } = usePostTypes(storeId);
  ...
  const filteredPosts = useMemo(() => {
    return posts?.filter((post: Post) => {
      ...
      const matchesPostType = !postTypeFilter || post.post_type.id === postTypeFilter;
      const matchesStatus = !statusFilter || post.status === statusFilter;
      const matchesContentType = !contentTypeFilter || post.content_type === contentTypeFilter;
        ...
        <TableBody>
          {filteredPosts?.map((post: Post) => (
            <TableRow key={post.id}>
              ...
              <TableCell>{post.created_by.username}</TableCell>
              <TableCell>{post.updated_by.username}</TableCell>
              <TableCell>
