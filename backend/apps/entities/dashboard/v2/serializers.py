...
class DashboardEntityActionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'content_types', 'is_active', 'is_public', 'allow_anonymous', 'metadata',
            'store', 'interaction_count', 'active_interaction_count', 'unique_user_count',
            'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'created_at', 'updated_at', 'interaction_count', 
            'active_interaction_count', 'unique_user_count'
        ]
    ...
    def get_unique_user_count(self, obj):
        ...
        return EntityInteraction.objects.filter(action=obj, is_active=True).values('user').distinct().count()
...
class DashboardEntityActionCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = EntityAction
        fields = [
            'name', 'slug', 'description', 'action_type', 'icon', 'color',
            'content_types', 'is_active', 'is_public', 'allow_anonymous', 'metadata'
        ]
...
class DashboardEntityInteractionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'action', 'action_name', 'action_slug', 'action_type',
            'user', 'username', 'user_email', 'content_type', 'object_id', 'content_type_name',
            'value', 'rating', 'is_active', 'ip_address', 'user_agent',
...
class DashboardBulkInteractionSerializer(serializers.Serializer):
    ...
    def validate_interaction_ids(self, value):
        ...
        if store:
            existing_ids = EntityInteraction.objects.filter(
                id__in=value,
                store=store
            ...
            if missing_ids:
                raise serializers.ValidationError(
                    f"Interaction IDs not found: {list(missing_ids)}"
                )
