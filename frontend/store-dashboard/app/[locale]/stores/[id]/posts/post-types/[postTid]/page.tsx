...
export default function PostTypeDetailPage({ params }: { params: Promise<{ id: string; locale: string; postTid: string }> }) {
  const router = useRouter();
  const resolvedParams = React.use(params);
  const id = resolvedParams.postTid;
  const { data: postType, error, isLoading } = usePostType(postTypeId);
