...
class LogService:
    ...
    def get_store_analytics(store, days=30):
        ...
        # Security events
        security_events = (
            LogQueryHelper.get_base_query(store, since).filter(is_suspicious=True).count()
        )

        # Event type distribution
        event_distribution = (
            LogQueryHelper.get_base_query(store, since)
            .values("event_type")
    ...
    def get_user_activity(store, user, days=30):
        ...
        activities = BaseTenantCRUDService.filter(
            store=store, user=user, created_at__gte=since
        ).order_by("-created_at")

        # Activity summary
        activity_summary = (
            activities.values("event_type").annotate(count=Count("id")).order_by("-count")
        )
    ...
    def get_entity_history(store, entity_type, entity_id, days=30):
        ...
        return list(
            BaseTenantCRUDService.filter(
                store=store, entity_type=entity_type, entity_id=entity_id, created_at__gte=since
            ).order_by("-created_at")
        )
    ...
    def get_real_time_metrics(store, minutes=5):
        ...
        recent_logs = BaseTenantCRUDService.filter(store=store, created_at__gte=since)
        ...
        avg_response_time = (
            recent_logs.filter(event_type="PAGE_VIEW", duration_ms__isnull=False).aggregate(
                avg=Avg("duration_ms")
            )["avg"]
            or 0
        )
