...
class SMTPConfigDashboardSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SMTPConfig
        fields = [
            'id', 'name', 'host', 'port', 'username', 'password', 'use_ssl', 'use_tls',
            'is_active', 'is_default', 'email_count', 'success_rate', 'created_at', 'updated_at'
        ]
...
class EmailLogDashboardSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = EmailLog
        fields = [
            'id', 'smtp_config', 'smtp_config_name', 'to_email', 'subject', 'body',
