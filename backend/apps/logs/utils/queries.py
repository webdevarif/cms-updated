...
class LogQueryHelper:
    ...
    def get_page_view_stats(store, since=None):
        ...
        return {
            ...
            'top_pages': list(base_query.values('page_url').annotate(
                views=Count('id')
            ).order_by('-views')[:10])
        }
    ...
    def get_bounce_rate(store, since=None):
        ...
        bounce_sessions = base_query.values('session_id').annotate(
            page_views=Count('id')
    ...
    def detect_suspicious_patterns(store, since=None, threshold=5):
        ...
        # Failed logins
        failed_logins = base_query.filter(event_type='LOGIN_FAILED').values('ip_address').annotate(
            count=Count('id')
        ).filter(count__gt=threshold)
        
        # Rapid requests
        rapid_requests = base_query.filter(event_type='PAGE_VIEW').values('ip_address').annotate(
            count=Count('id')
        ).filter(count__gt=1000)
        
        return {
            'failed_logins': list(failed_logins),
            'rapid_requests': list(rapid_requests)
        }
