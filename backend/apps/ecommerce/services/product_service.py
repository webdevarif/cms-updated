...
class ProductService:
    ...
    def get_available_products(store):
        ...
        return Product.objects.filter(store=store, status="active").prefetch_related(
            "variants", "images"
        )
    ...
    def get_product_with_variants(product_id):
        ...
        return Product.objects.filter(id=product_id).prefetch_related("variants", "images").first()
...
class CartService:
    ...
    def get_or_create_cart(store, user=None, session_key=None):
        ...
        if user:
            ...
            cart, created = Cart.objects.get_or_create(
                store=store, session_key=session_key, status="active"
            )
        return cart
