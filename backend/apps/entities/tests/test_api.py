...
class EntityViewSetTests:
    """Entity viewset tests for all roles"""
    ...
    def test_create_entity_as_admin(self, authenticated_client):
        """Test creating entity as admin"""
        response = authenticated_client.post('/v2/api/entities/', {
            'name': 'Test Entity',
            'slug': 'test-entity'
        })
        assert response.status_code == 201
    
    def test_create_entity_as_anonymous(self, anonymous_client):
        """Test creating entity as anonymous (should fail)"""
        response = anonymous_client.post('/v2/api/entities/', {
            'name': 'Test Entity',
            'slug': 'test-entity'
        })
