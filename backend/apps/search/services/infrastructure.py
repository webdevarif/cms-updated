...
from elasticsearch_dsl import A, Document, Index, Search
...
class SearchService:
    ...
    def search(store, query, search_type="all", filters=None, page=1, page_size=20):
        ...
        try:
            ...
            # Add highlighting
            s = s.highlight(
                "title", "content", "description", fragment_size=150, number_of_fragments=3
            )
            ...
            for facet in index.facets:
                ...
                if field in response.aggregations:
                    ...
                    facets[field] = [
                        {"value": bucket.key, "count": bucket.doc_count} for bucket in buckets
                    ]
    ...
    def get_suggestions(store, query):
        ...
        try:
            ...
            s = s.suggest("prefix_suggest", query, prefix={"field": "title", "size": 10})
            ...
            if response.suggest.title_suggest:
                for suggestion in response.suggest.title_suggest:
                    for option in suggestion.options:
                        suggestions.append({"text": option.text, "score": option.score})
    ...
    def _create_index_with_mappings(index_name, content_type):
        ...
        mappings = {
            "properties": {
                "title": {
                    "type": "text",
                    "fields": {"suggest": {"type": "completion"}, "keyword": {"type": "keyword"}},
                },
                "content": {"type": "text"},
            ...
            data.update(
                {
                    ...
                    "description": getattr(obj, "seo_description", "")
    ...
    @staticmethod
    def track_search(
        store, query, search_type, results_count, filters=None, user=None, duration_ms=None
    ):
        """
