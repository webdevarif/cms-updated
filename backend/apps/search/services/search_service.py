...
class SearchService:
    ...
    def search(
        ...
        try:
            ...
            search_query = SearchService._build_search_query(
                query, types, store, user, comment_type, min_rating
            )
    ...
    def _build_search_query(
        query, types=None, store=None, user=None, comment_type=None, min_rating=None
    ):
    ...
    def _add_facets(search_query):
        ...
        search_query.aggs.bucket(
            "recent", "date_histogram", field="created_at", calendar_interval="day"
        )
    ...
    def _process_results(response):
        ...
        for hit in response:
            result = {
                ...
                "content": getattr(hit, "content", "")[:200] + "..."
                if hasattr(hit, "content") and len(hit.content) > 200
                else getattr(hit, "content", ""),
                "url": getattr(hit, "url", ""),
                ...
                "metadata": {"index": hit.meta.index, "id": hit.meta.id, "score": hit.meta.score},
            }
