...
class UserService:
    ...
    def create_user_store_owner(store, email, username, password):
        ...
        with transaction.atomic():
            from apps.accounts.models import StoreUser, User
            ...
            StoreUser.objects.create(user=user, store=store, role="owner", is_active=True)
    ...
    def add_user_to_store(user, store, role="customer"):
        ...
        store_user, created = StoreUser.objects.get_or_create(
            user=user, store=store, defaults={"role": role, "is_active": True}
        )
    ...
    def get_store_staff(store):
        ...
        return StoreUser.objects.filter(
            store=store, role__in=["owner", "admin", "manager", "staff"], is_active=True
        )
