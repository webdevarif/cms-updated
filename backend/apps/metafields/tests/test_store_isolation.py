...
def test_metafielddefinition_store_isolation(store_a, store_b):
    ...
    def_a = MetafieldDefinition.objects.create(
        store=store_a, namespace="test", key="color", type="text"
    )
    def_b = MetafieldDefinition.objects.create(
        store=store_b, namespace="test", key="color", type="text"
    )
