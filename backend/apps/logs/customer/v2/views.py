...
class CustomerLogViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    @extend_schema(
        summary="List store logs",
        description="Get paginated list of store-specific logs"
    )
    ...
    @extend_schema(
        ...
        description="Get statistics about store logs"
    )
    ...
    @extend_schema(
        summary="Export store logs",
        description="Export store logs as CSV"
    )
    @action(detail=False, methods=['get'])
