    ...
    try:
        ...
        try:
            ...
            if keys:
                ...
                logger.info(f"Cleared {len(keys)} translation cache entries with pattern: {pattern}")
            else:
...
def warm_store_translations(store_id):
    ...
    try:
        ...
        translations = Translation.objects.filter(store_id=store_id).select_related('key', 'language')
