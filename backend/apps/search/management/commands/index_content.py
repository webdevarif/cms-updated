...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--type", type=str, required=True, help="Content type to index (Page, Post, Product)"
        )
    ...
    def _index_products(self, store, batch_size):
        ...
        for product in products:
            ...
            success = SearchService.index_document(
                store=store, document_type="Product", document_id=str(product.id), data=data
            )
    ...
    def _index_posts(self, store, batch_size):
        ...
        for post in posts:
            ...
            success = SearchService.index_document(
                store=store, document_type="Post", document_id=str(post.id), data=data
            )
    ...
    def _index_pages(self, store, batch_size):
        ...
        for page in pages:
            ...
            success = SearchService.index_document(
                store=store, document_type="Page", document_id=str(page.id), data=data
            )
