...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--batch-size", type=int, default=100, help="Batch size for updates (default: 100)"
        )
        parser.add_argument(
            "--models", type=str, help="Comma-separated list of models to update (default: all)"
        )
        ...
        while processed < total:
            with transaction.atomic():
                ...
                for product in products:
                    ...
                    if product.metadata:
                        metadata_text = " ".join(
                            str(v) for v in product.metadata.values() if isinstance(v, str)
                        )
                        if metadata_text:
    ...
    def update_pages(self, batch_size):
        ...
        while processed < total:
            with transaction.atomic():
                # Get batch of pages
                pages = Post.objects.filter(post_type__slug="page")[
