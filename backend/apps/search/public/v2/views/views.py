"""
Public search API views - endpoint-focused only.
"""
...
@extend_schema(
    ...
    description=(
        "Full-text search with Elasticsearch tuning, facets, highlighting, and suggestions. "
        "Provides real-time search capabilities for public content."
    ),
)
class SearchViewSet(viewsets.ViewSet):
        ...
        try:
            ...
            for result in search_result["results"]:
                ...
                if result["type"] == "Comment":
                    processed_result[
                        "url"
                    ] = f"/posts/{result.get('post_id')}/#comment-{result['id']}"
                elif result["type"] == "Review":
                    processed_result[
                        "url"
                    ] = f"/products/{result.get('product_id')}/#review-{result['id']}"
                ...
                if result["type"] in ["Comment", "Review"]:
                    ...
                    if result["type"] == "Review":
                        ...
                        processed_result["verified_purchase"] = result.get(
    ...
    def facets(self, request):
        ...
        try:
            ...
            return Response(
                {"error": f"Facets failed: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
