...
from .serializers import (
    ProductCategoryDashboardSerializer, DashboardProductSerializer, ProductVariantDashboardSerializer,
    CartDashboardSerializer, CartItemDashboardSerializer, DashboardCollectionSerializer,
    DashboardInventorySerializer, OrderDashboardSerializer, OrderItemDashboardSerializer,
    CouponCampaignDashboardSerializer, CouponDashboardSerializer,
    PaymentMethodDashboardSerializer, PaymentDashboardSerializer,
...
class DashboardProductViewSet(viewsets.ModelViewSet):
    ...
    def analytics(self, request, pk=None):
        ...
        total_revenue = CartItem.objects.filter(
            product=product,
        ...
        total_orders = CartItem.objects.filter(
            product=product,
            cart__status='completed'
        ).count()
...
class DashboardCollectionViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        description="Partially update collection"
    )
    def partial_update(self, request, *args, **kwargs):
...
class OrderItemDashboardViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return OrderItem.objects.filter(
            order__store=self.request.store
...
class CouponCampaignDashboardViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Filter by current user's store"""
        return CouponCampaign.objects.filter(
            store=self.request.store
        ).select_related('store')
    ...
    def generate_coupons(self, request, pk=None):
        ...
        for i in range(count):
            coupon = Coupon.objects.create(
                campaign=campaign,
                code=f"{campaign.name.upper()}-{i+1:03d}"
            )
...
class CouponDashboardViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Coupon.objects.filter(
            campaign__store=self.request.store
        ).select_related('campaign', 'customer', 'order')
...
class EcommerceAnalyticsViewSet(viewsets.ViewSet):
    ...
    def analytics(self, request):
        ...
        try:
            ...
            new_customers = customers.filter(created_at__range=[start_date, end_date]) if start_date and end_date else 0

            # Category performance
            category_sales = OrderItem.objects.filter(
                order__store=store,
                order__status='completed',
                order__created_at__range=[start_date, end_date] if start_date and end_date else [timezone.now() - timedelta(days=30), timezone.now()]
            ...
            if start_date and end_date:
                ...
                daily_revenue = completed_orders.annotate(
            ...
            analytics_data = {
                'overview': {
                    ...
                    'new_customers': new_customers if isinstance(new_customers, int) else new_customers.count()
                },
                'revenue': {
                ...
                'products': {
                    ...
                    'publish_rate': (published_products / total_products * 100) if total_products > 0 else 0
                },
                'categories': categories,
                'customers': {
                    'total': total_customers,
                    'new': new_customers if isinstance(new_customers, int) else new_customers.count(),
...
class ReviewDashboardViewSet(viewsets.ModelViewSet):
    ...
    def analytics(self, request):
        ...
        try:
            ...
            top_products = Review.objects.filter(
                product__store=store if store else Q(),
                is_approved=True,
                created_at__gte=timezone.now() - timezone.timedelta(days=days)
            ...
            for i in range(days, 0, -7):  # Weekly data points
                ...
                week_end = timezone.now() - timezone.timedelta(days=i-14) if i > 7 else timezone.now()
            ...
            if approved_only:
                analytics = {k: v for k, v in analytics.items() if 'approved' in k.lower() or 'total' in k.lower() or 'average' in k.lower()}
            ...
            full_analytics = {
                **analytics,
                ...
                'time_range': {
                    'start_date': (start_dt or (timezone.now() - timezone.timedelta(days=days))).isoformat(),
