...
class CacheCustomerViewSet(viewsets.ModelViewSet):
    ...
    def set_key(self, request):
        ...
        if not key or value is None:
            return Response(
                {"error": "Key and value are required"}, status=status.HTTP_400_BAD_REQUEST
            )

        try:
            CacheService.set(
                key=key, value=value, store=self.request.store, ttl=ttl, user=request.user
            )
    ...
    def get_key(self, request):
        ...
        try:
            ...
            if value is not None:
                ...
                return Response({"error": "Cache key not found"}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
