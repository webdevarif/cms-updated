...
class MetafieldDashboardViewSet(viewsets.ModelViewSet):
            ...
            with transaction.atomic():
                ...
                log_event_async.delay({
                    ...
                    'metadata': {
                        ...
                        'affected_count': updated if 'updated' in locals() else deleted if 'deleted' in locals() else 0,
                ...
                return Response({
                    ...
                    'found': total_found,
                    'affected': updated if 'updated' in locals() else deleted if 'deleted' in locals() else 0
                })
    ...
    def analytics(self, request):
        ...
        try:
            ...
            content_types = base_queryset.values('content_type__model').annotate(
                count=Count('id'),
            ...
            for ct in content_types:
                content_type_data.append({
                    ...
                    'visibility_rate': (ct['visible_count'] / ct['count'] * 100) if ct['count'] > 0 else 0
                })

            # Metafield definition usage
            definition_usage = base_queryset.values(
            ...
            stores = base_queryset.values('definition__store__name', 'definition__store__id').annotate(
            ...
            field_types = base_queryset.values('definition__field_type').annotate(
                count=Count('id'),
                percentage=(Count('id') * 100.0 / total_metafields) if total_metafields > 0 else 0
            ).order_by('-count')
            ...
            # Namespace analysis
            namespace_analysis = base_queryset.values('definition__namespace').annotate(
                total_metafields=Count('id'),
            ...
            analytics_data = {
                'overview': {
                    ...
                    'visibility_rate': (visible_metafields / total_metafields * 100) if total_metafields > 0 else 0,
                ...
                'trends': {
                    'activity': activity_trends
                },
                'usage_metrics': {
                    'avg_metafields_per_content_type': total_metafields / len(content_type_data) if content_type_data else 0,
