...
class CacheStatsPublicViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    @extend_schema(
        summary="Get cache statistics", description="Get public cache performance statistics"
    )
    ...
    def summary(self, request):
        ...
        # Get latest stats
        latest_stats = CacheStats.objects.filter(interval="daily").order_by("-period_start").first()

        if not latest_stats:
            return Response(
                {"total_keys": 0, "hit_ratio": 0, "memory_usage": 0, "avg_response_time": 0}
            )
